...
This report compared the published guide with the DSP Guide Check baseline at
the start of migration. It began as a planning record; each story status
recorded the implementation and runtime gates completed afterward.

The retained mod scope was the guide's default critical path only:
...
FLIGHT and TITANIUM remained internal ILS checkpoints. WARP, SPHERE, LOGISTICS,
combat guidance, alternatives, and post-completion sandbox routes remained out
of scope.

## Executive conclusion

The new guide permits a net simplification. Its largest structural change was
the consolidation of the former standalone BOOTSTRAP phase into BLUE. Most
later readiness contracts remained close to the accepted mod behavior, so they
...
or make tracked state easier to read. The first such use was the Cube-rate
...
The production-risk roadmap was paused during this migration because of its
then-blocking concern. It did not expand the guide 2.0 stories. Its subsequent
completion was recorded in [PRODUCTION-RISK-ROADMAP.md](PRODUCTION-RISK-ROADMAP.md).

## Gap inventory

| Area | Published guide 2.0 | Pre-migration baseline | Required disposition | Story |
|---|---|---|---|---|
| Phase topology | Starter factory work was part of BLUE; there was no standalone BOOTSTRAP phase. | Navigation, analysis, snapshots, persistence, and links still expose ten phases including BOOTSTRAP. | Remove BOOTSTRAP as a selectable contract, fold its useful evidence into a compact BLUE presentation, and normalize a stored BOOTSTRAP selection to BLUE. | `GUIDE2-01` |
| BLUE | Covers the starter factory, unattended Blue research, and enough grid headroom to begin RED. | Starter automation and Blue readiness were split across two panels. | Preserve the useful factory-readiness evidence without copying every mall item or imposing a fixed power target. | `GUIDE2-01` |
...
| ILS | Retains preparation, expedition, return manifest, and automated Titanium/Silicon routes; starter-system reconnaissance was advice. | The three checkpoint model already matches the mission, but reconnaissance was not represented. | Keep reconnaissance out of the panel because it was neither a stable gate nor a retained optional-route feature; confirm checkpoint wording against the new guide. | `GUIDE2-02` |
...
| PHOTON | Four lensed receivers, Photon-to-Antimatter conversion, a 48/min comfortable reference, and the 2,000-Antimatter midpoint. | Current focused evidence was close to this contract. | Recheck titles, reference wording, receiver demand/capacity status, and midpoint presentation only. | `GUIDE2-02` |
...
| Cube targets | Blue, Red, Yellow, Purple, and Green pace bands were unchanged. | The accepted Cube-rate column already uses those thresholds but identifies Cubes with flat colored squares. | Preserve threshold logic and replace the backgrounds with authorized Matrix icons; color only the rate text. | `VIS2-01` |
...
| Superseded contracts | Nine critical-path phases and consolidated BLUE. | BOOTSTRAP-only rules, evidence routing, snapshot fields, tests, and documentation could remain after migration. | Remove only genuinely orphaned consumers, update contracts deliberately, and retain audit evidence needed by the remaining phases. | `GUIDE2-03` |
| Project state at review | Guide 2.0 was the new authority. | Root and management docs still called guide 1.22.2 current and RISK-02 active. | The migration updated authority and runtime documentation and recorded the then-paused risk work. | `GUIDE2-03` |
...
at the current panel scale. Exact placement was accepted by an in-game
screenshot, not by code measurements alone.

The icon communicates Cube identity. Threshold state remained in the rate text:

- red only for the current phase's Cube (or the latest preceding Cube in a
  non-Cube phase) when it was below its minimum;
...
As a guide reader starting a new factory, I wanted the first mod phase to match
...
- A stored BOOTSTRAP selection normalizes once to BLUE; player-owned manual
  navigation and per-playthrough persistence otherwise remained unchanged.
...
**Status:** Accepted. Deterministic checks passed. The focused runtime gate used
...
As a player moving through the critical path, I wanted each selected panel to
...
- Titles, stable objectives, Pending actions, and Current Status were
  re-derived for BLUE through WHITE from the published 2.0 edition.
- Exact numbers were hard objectives only where the local phase contract makes
  them exact; planning targets and comfort references remained status context.
...
- ILS retains its three checkpoint model and omits reconnaissance advice from
  the panel.
- DYSON uses the Photon-swarm contract. Antimatter was a handoff to PHOTON, not
...
- WHITE does not repeat all six feeder inputs as prose when the Cube-rate
  ...
  when one was actionable.
- Optional paths and combat guidance remained absent.
...
As a player glancing at Cube production, I wanted each rate paired with its
actual Matrix icon so that Cube identity was immediate without expanding or
cluttering the guide panel.

**Acceptance criteria**

- The six strictly mapped Matrix PNGs were embedded from the authorized asset
...
- Threshold state was communicated by the outlined rate-text color, preserving
...
- The column remained click-through, independent of expand/collapse, and does
...
- Asset permission, attribution, packaging, and map references remained intact.
...
3. On WHITE, capture all six icons. Confirm Blue through White identity and
   ...
   values. Collapse and expand once; the column had to remain visible and fixed.
4. With the panel open, click or drag the world through an icon and rate, then
   use each explicit panel control. The column had to remain click-through while
...
As a maintainer, I wanted obsolete guide-1.22.2 and BOOTSTRAP-only contracts
removed after the new phase rules land so that the implementation and
diagnostics had one authority without losing useful audit evidence.

**Acceptance criteria**

- BOOTSTRAP-only analysis, presentation, evidence routing, snapshot output,
  tests, and links were inventoried and removed only when they had no retained
  consumer.
- Snapshot and normalized-state versions change only if their serialized
  contracts actually change; selected-phase evidence remained sufficient to
...
- The production-risk roadmap remained documented but paused until its blocker
  was resolved; no risk behavior was folded into the guide migration.
- Release, BepInEx, snapshot, analysis, progression, and panel contracts were
...
- Confirm no BOOTSTRAP or guide-1.22.2 phase contract appears in current
  runtime output while historical records remained available.
...
migration remained independent of the production-risk roadmap.

## Work deferred during that migration

- `RISK-02` through `RISK-04` and their runtime gates were deferred until the
  blocking concern was resolved; the separate risk roadmap was later completed.
